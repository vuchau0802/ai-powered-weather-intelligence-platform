...
const openWeatherErrorMessage =
  "OpenWeather rejected the API key. Check OPENWEATHER_API_KEY and make sure it is active for the Current Weather and 5 Day Forecast APIs.";

const getErrorStatus = (err: unknown) => {
  return axios.isAxiosError(err) ? err.response?.status : undefined;
};

const getOpenWeather = async <T>(url: string, params: Record<string, string | number>) => {
  try {
...
export const getWeather = async (city: string) => {
  ...
  if (!weatherKey) throw new WeatherServiceError("Missing OPENWEATHER_API_KEY in environment", 500);
  ...
  if (process.env.ENABLE_GEOCODING === "true" && geoKey) {
    ...
    const currentByCity = await getOpenWeather<any>("https://api.openweathermap.org/data/2.5/weather", {
      q: city,
      units: "imperial",
      appid: weatherKey,
    });
  ...
  const [currentWeather, forecastWeather] = await Promise.all([
    getOpenWeather<any>("https://api.openweathermap.org/data/2.5/weather", {
      lat,
      lon,
      units: "imperial",
      appid: weatherKey,
    }),
    getOpenWeather<any>("https://api.openweathermap.org/data/2.5/forecast", {
      lat,
      lon,
      units: "imperial",
      appid: weatherKey,
  ...
  let airQuality: { aqi: number; pm2_5: number; pm10: number } | null = null;

  if (process.env.ENABLE_AIR_QUALITY === "true") {
    try {
      const air = await axios.get("https://air-quality-api.open-meteo.com/v1/air-quality", {
  ...
  if (process.env.ENABLE_YOUTUBE === "true" && ytKey) {
    try {
      ...
      videos = yt.data.items.map((v: any) => ({
  ...
  if (aqi > 150) insight = "Air quality is very unhealthy. Avoid outdoor activity.";
  else if (aqi > 100) insight = "Air quality is unhealthy for sensitive groups. Limit outdoor activity.";
  else if (current.main.temp > 95) insight = "High heat warning. Stay hydrated and avoid peak sun hours.";
  else if (current.main.temp < 32) insight = "Freezing conditions. Dress in layers and watch for ice.";
  else if (current.weather[0].main === "Rain") insight = "Rain expected. Bring an umbrella.";
  else if (current.weather[0].main === "Snow") insight = "Snow expected. Allow extra travel time.";
  else if (current.main.temp > 82) insight = "Warm day ahead. Sunscreen recommended.";
